...
- `AGENTS.md` всегда генерируется **на английском** (единый стандарт для всех репозиториев).
- Язык общения с пользователем задаётся в `.sddkit/config.toml` через `[sddkit].locale` и должен ему соответствовать.
